...
public class KnowledgeBaseVectorService {
    ...
    public List<Document> similaritySearch(String query, List<Long> knowledgeBaseIds, int topK, double minScore) {
        ...
        try {
            ...
            if (knowledgeBaseIds != null && !knowledgeBaseIds.isEmpty()) {
                // 把 List<Long> 转成 Spring AI VectorStore 识别的 DSL 字符串，比如 "kb_id in ['1','3','5']"。
                // filterExpression(...)：把这个表达式交给向量库实现，作为 “按 metadata 过滤文档” 的条件，从而实现“多知识库里只搜指定几个”的效果。
    ...
    private String buildKbFilterExpression(List<Long> knowledgeBaseIds) {
        // 在把文档写入向量库时，你自己在 Document 的 metadata 里放了一个键值：
        // key："kb_id"
        // value：对应的知识库 ID（Long 或 String）
