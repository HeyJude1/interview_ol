...
public class InterviewPersistenceService {
    ...
    public InterviewAnswerEntity saveAnswer(String sessionId, int questionIndex,
        ...
        // 其实这是Optional.orElseGet的用法，如果找到就返回找到的，如果没找到就创建一个新的
        // 因为如果一场面试创建的时候,只是写入到了redis中,数据库中来不及写入
        // 然后用户马上做题提交答案,此时还要把答案保存到数据库中,发现没有这场面试的信息,就再尝试创建一个新的面试会话记录
        InterviewAnswerEntity answer = answerRepository
            ...
            .orElseGet(() -> {
                ...
                // session.get()就是从Optional中提取出实体对象
        ...
        // JpaRepository会自带save方法,用于保存该实体
    ...
    /**
     * 根据会话ID获取会话
     * @return Optional<InterviewSessionEntity>
     */
