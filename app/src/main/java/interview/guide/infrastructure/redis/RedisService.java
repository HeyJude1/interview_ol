...
public class RedisService {
    ...
    public <T> T get(String key) {
        // getBucket(key) 返回一个 RBucket<T> 对象，它代表 Redis 中的一个键值对
        // get() 方法从 Redis 中获取该键对应的值:
        // 到 Redis 里用这个 key 取出对应的 单个 value（底层是 bytes）；
        // 用 Redisson 配置好的 codec 反序列化成 Java 对象；
        // 返回这个对象，类型是 T
        RBucket<T> bucket = redissonClient.getBucket(key);
        return bucket.get();
    }
    ...
    public boolean streamConsumeMessages(
        ...
        // 获取 Stream 对象,通过 RedissonClient匹配streamKey和StringCodec来获取
        RStream<String, String> stream = redissonClient.getStream(streamKey, StringCodec.INSTANCE);

        // 使用阻塞读取，让 Redis 服务端等待消息
        // 在这个streamKey下，使用groupName和consumerName来读取消息
        // 每个group中都是独立的消费进度，互不影响,初始状态下都是完整的streamKey中的所有消息
        ...
        for (Map.Entry<StreamMessageId, Map<String, String>> entry : messages.entrySet()) {
            // 这个getKey其实是stream消息中streamKey的值的键MessageId,所以stream中的消息其实是嵌套了2层的键值对
